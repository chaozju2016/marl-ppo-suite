...
def _transform_data(data_np: np.ndarray, device: torch.device, sequence_first: bool = False) -> torch.Tensor:
    ...
    if sequence_first:
        ...
        reshaped_data = data_np.transpose(1, 0, 2).reshape(-1, *data_np.shape[2:]).copy()
...
class RecurrentRolloutStorage:
    ...
    def __init__(self, n_steps, n_agents, obs_dim, action_dim, state_dim, hidden_size,
        ...
        Args:
            ...
            use_value_norm (bool): Whether to use value normalization
        ...
        self.active_masks = np.ones((n_steps + 1, n_agents, 1), dtype=np.float32) # 0 if agent dead, 1 otherwise
    ...
    def compute_returns_and_advantages(self,
        ...
        Args:
            ...
            value_normalizer: Value normalizer object (optional)
        """
        ...
        # If using value normalization, denormalize values
        if self.use_value_norm and value_normalizer is not None:
            values = value_normalizer.denormalize(self.values)
        else:
            values = self.values
        # values = self.values # no need to denormalize raw values
        ...
        if use_gae:
            ...
            for step in reversed(range(self.n_steps)):
                ...
                if np.any(truncated_mask):
                    ...
                    adjusted_rewards[truncated_mask] += gamma * values[step + 1][truncated_mask]

                # Calculate delta (TD error) with adjusted rewards
                delta = (
                    adjusted_rewards +
                    gamma * values[step + 1] * self.masks[step + 1] -
                    values[step]
                ) # [n_agents]
            ...
            returns[:-1] = advantages + values[:-1] # [n_agents]
            returns[-1] = values[-1] # [n_agents]
    ...
    def get_minibatches_batch_first(self, num_mini_batch, data_chunk_length = 10):
        ...
        for i in range(0, len(all_starts), mini_batch_size):
            ...
            sequences = {
                ...
                'returns': [], 'masks': [], 'active_masks': [], 'action_log_probs': [],
            ...
            for start_idx in batch_chunk_starts:
                ...
                sequences['active_masks'].append(self.active_masks[start_idx:end_idx])
    ...
    def get_minibatches_seq_first(self, num_mini_batch, data_chunk_length = 10):
        ...
        mini_batch_size = max(1, max_data_chunks // num_mini_batch)  # e.g., 200 // 1 = 200
